...
## 2026-08-19, direct follow-up ("dann versuche eine andere strategie... wir können das nicht live tracken"): all 15 real `$31AD` callers traced statically for HOW they set HL, uncovering a real, general per-actor "adjacent-grid-cell + list lookup" dispatch primitive -- and a decisive, honest negative for index 5744 specifically

Direct redirect away from live tracking entirely: statically re-scanned the WHOLE ROM (all 16 banks, not just banks 0-4) for real call/jump sites targeting `$31AD`/`$3213`, using `tools/rom/disasm.py` plus a self-consistent forward-disassembly-alignment check (try successive backward start points; keep the one whose decode lands exactly on the call site with no instruction straddling it) to validate each hit as real code, not a coincidental byte pattern. Result: exactly 15 `$31AD` callers + 3 `$3213` callers, confirming the earlier "banks 0-4" scope was a genuine, complete result, not an accidental restriction.

**Backward-disassembled all 15 callers' own immediate HL setup.** 8 of the 15 (`$3E72`, `$51C4`, `$5210`, `$8A6D`, `$8EDD`, `$B40C`, `$BD04`, and part of `$101FD`'s own chain) load a small literal `LD HL,imm16` (values like `0x0007`/`0x000B`/`0x001F`/`0x0003`) or an explicit `H=0` -- these can NEVER reach bank 14, since `$31CC`'s own `H`-nonzero check routes them to the "3 special-case fixed buffers" path instead of `$3282`'s table lookup. Only a genuinely different subset can reach large indices: `$173F`, `$247C`/`$24A0`/`$24CD` (three call sites inside ONE function), and `$C3A4` set HL via a real memory dereference (`LD A,(HL+)/LD H,(HL)/LD L,A`), not a literal; `$973A` and `$C5F3` resist this specific static technique (H's real origin sits outside the traced window).

**A genuinely new structural discovery**: traced `$247C`/`$24A0`/`$24CD`'s own shared WRAM pointer (`$C3FE`/`$C3FF`) back to its 4 real write sites (all bank-0 fixed, clustered `$2689`-`$2768`), which trace further into a real, general **per-actor "adjacent-grid-cell" primitive** (entry `$2617`): given a current grid position (`D`,`E`) and a facing/direction byte (bits 7/0/1/2/3 tested individually), it computes the neighboring cell's position, bounds-checked against `$C3FA`/`$C3FB` (a real row/column limit -- almost certainly the current actor's own associated grid's dimensions). `$25D1`/`$25F6` then compute a row-major cell index (`D*$C3FB+E`, doubled twice = `*4`) into a 4-byte-per-cell grid (`DE`=a pointer, `HL`=a secondary value) rooted at a per-actor blob pointer (`$C3F2`/`$C3F3`) that itself comes from an 11-byte-record table in BANK 8 (indexed by an as-yet-unidentified "actor/object type" byte) -- the SAME bank the `$4F11` transition-index table lives in. The grid cell's `DE` field is stored to `$C3FE`/`$C3FF` and later dereferenced (after re-pushing the actor's own bank via `$C3F0`) to produce the final value passed to `$31AD` -- for `$24A0` with a `+1` offset and `$24CD` with a `+2` offset applied to the raw dereferenced word (`INC HL` once/twice) before the call; `$247C` uses it raw. `$173F` turns out to share this SAME mega-function (entered at `$2617` itself, not a separate simpler table) -- its own downstream code performs a SECOND, linear 3-byte-record (key+word) list search over a table reached via the grid lookup's own result, keyed by a transformed room/context byte, before producing its final HL. This is a real, general "what's the exit/object in this direction, for this context" dispatch primitive, most likely reused for door/warp AND other actor-interaction purposes -- not proven room-transition-specific, but the clearest real candidate mechanism found so far for how transitions could be data-driven per room/actor rather than hardcoded.

**Decisive, honest negative for the specific target this session has been chasing (table index 5744)**: since all of `$247C`/`$24A0`/`$24CD`/`$173F`'s own final HL comes from a **direct memory dereference** (no arithmetic beyond the `+1`/`+2` `INC HL` already accounted for), the raw bytes `0x70 0x16` (5744), `0x6F 0x16` (5743, the `+1`-corrected value), or `0x6E 0x16` (5742, the `+2`-corrected value) would have to exist somewhere in the ROM as literal stored data for any of these paths to produce 5744. **A full 256KB ROM byte-pattern search found zero genuine occurrences of all three.** One apparent hit for the `+1` case (file `0x1E9FC`, bank 7) was checked directly and correctly diagnosed as a coincidental MISALIGNED byte pair straddling two real, unrelated entries of a genuine, monotonically-increasing sorted 16-bit pointer table (bank 7's own, real, all values in the plausible `$6Exx`-`$6Fxx` range) -- the same "false positive inside real data, not code" trap this project has caught before, this time inside a pointer table rather than executable code. A broader, unfiltered literal search across the full 1363-index bank-14-reaching candidate set found 610 raw hits, but this measure is not discriminating enough to be useful on its own (a ~4% per-instruction coincidence rate across thousands of real, unrelated `LD reg,imm16` instructions in a 256KB ROM) -- reported honestly as noise, not pursued further without a way to narrow it.

**Honest, complete conclusion for this pass**: real, structural, previously-undocumented progress (the per-actor grid+list dispatch primitive) plus a clean, decisively negative result specifically for index 5744 -- it is not reachable via a pure literal dereference through any of the traced call sites, meaning it must either come from further arithmetic this pass didn't uncover, from one of the two still-unresolved callers (`$973A`/`$C5F3`), from the bank-8 11-byte actor-type table's own still-unidentified index source, or genuinely isn't naturally reachable from currently-known game content at all. The mechanism-level finding from the previous entry (the redirect math itself, live-confirmed) stands unchanged; what remains open is specifically "what real game state naturally produces a bank-14-reaching HL value," now understood one to two levels deeper than before, still not fully closed. No production `src/` code changed; `luajit tests/run_tests.lua` unaffected.

## 2026-08-19, same-day direct follow-up (chosen strategy: "Bank-8-Actor-Tabelle statisch dumpen"): the `$26DC` family definitively RULED OUT as a bank-14 source; `$C3A4` traced into the game's generic per-frame active-actor-slot loop

Continuing the static-only investigation, chose to fully resolve the bank-8 11-byte record table's own index source rather than guess.

**`$26DC` (the real entry point of the `$247C`/`$24A0`/`$24CD`/`$173F` grid+list mechanism) has exactly 5 real static callers, ALL 5 passing the identical literal `A=0x07`** -- not a variable per-actor-type dispatch at all, a single fixed record. Computed the real bank-8 table entry for index 7 directly (file `0x2004D`): resolves to a real blob in bank 7 at file `0x1FBB1` (selector byte 0, row-bound 2, column-stride 16 -- a real 2x16 grid, 32 cells, 4 bytes/cell). **Dumped and dereferenced all 32 cells' own pointer field directly from ROM (fully static, no live emulation)**: every value is `0x0000`, `0x003D`, or `0xFFFF` -- sentinel/list-terminator-shaped, not remotely resembling a table index in the thousands. **Decisive, concrete negative**: this entire mechanism cannot produce a bank-14-reaching value for any currently-known call path, since its one and only real record never varies.

**`$C3A4` traced to the game's own generic per-frame active-actor loop.** Found via a real, general `$C4E0` WRAM structure (8 actor slots, 24 (`0x18`) bytes each) referenced from 6 real `LD HL,$C4E0` sites across banks 0 and 3: a "clear all slots" reset (`$27E9`), and a genuine "for each active slot, call the per-actor update routine" loop (`$C02C`) whose own per-slot processing (`$404A`, not yet traced) reads a real word field at slot offset `0x12` -- exactly matching `$C3A4`'s own earlier-traced `+0x12`/`+0x14` nested dereference. This confirms `$C3A4` is a real, generic per-active-actor field read, not actor-type-fixed like `$26DC` -- but finding what real, concrete values ever populate a slot's own fields requires locating the "spawn/populate" routine (not yet found; a literal-address search for `$C4E8`/`$C4E9`, slot 0's own offset-8 field, found zero hits, confirming slot addresses are always computed, never literal, consistent with a genuine per-slot loop rather than a handful of fixed instances).

**Honest status**: one of the four "large-HL-capable" callers is now definitively ruled out with concrete evidence (`$26DC`/`$247C` family). `$C3A4` is real and generic but requires finding the actor-slot "spawn" routine (and, ultimately, a per-room actor-placement table this project hasn't located) to make further static progress -- a well-defined, but substantially deeper, next research target. `$973A`/`$C5F3` remain untraced beyond the previous pass. No production `src/` code changed; `luajit tests/run_tests.lua` unaffected.

## 2026-08-19, same-day direct follow-up (chosen strategy: "$973A/$C5F3 zu Ende zurückverfolgen"): both remaining unresolved callers open into the game's large, general condition-dispatch system -- closing out the full 15-caller static trace

**`$973A`** resolves cleanly: its HL comes from `CALL $290D`'s own return value when nonzero (falls back to a fixed literal `0x0020` only when `$290D` returns exactly zero). `$290D` itself is a condition-gated dispatcher: `LD HL,0/CALL $28C2/RET NZ` (bail with HL=0 unless a real condition holds), then, depending on which of several already-differentiated entry points is reached, does `PUSH AF/LD A,<0x10-0x15>/JP $1F35` -- **the exact same shared jump table already seen this session at `$27CE` (indices 0-3, part of the earlier-ruled-out `$26DC` investigation)**, now confirmed used with a completely different index range (`0x10-0x15`) by a completely unrelated caller. This is decisive evidence `$1F35` is the game's own large, general-purpose condition/event dispatch table, reused throughout, not a small structure specific to any one mechanism -- cataloging it fully (to find what handler `0x10`-`0x15` actually return) is a substantially bigger undertaking than tracing a single caller, out of scope for this pass.

**`$C5F3`** resolves into a comparably complex direction-bit-tested per-actor table search (shape matching the earlier-documented `$2617` neighbor-lookup, but a structurally distinct table) -- backward-disassembling further surfaced a genuine alignment ambiguity between two candidate window starts (one landing cleanly on `0xc599`, another, equally self-consistent so far, implying the real instruction boundary is `0xc59a`) that needs more careful resolution than a quick pass allows; not resolved this session.

**Final, complete, honest conclusion for the full 15-caller static trace, closing out this thread**: 8 of 15 use small literals and can never reach bank 14 (confirmed earlier pass). Of the remaining 7 (`$173F`, `$247C`, `$24A0`, `$24CD`, `$C3A4`, `$973A`, `$C5F3`) -- 4 (`$173F`/`$247C`/`$24A0`/`$24CD`, all sharing the `$26DC` entry point) are now DEFINITIVELY RULED OUT (their one real bank-8 record's grid dereferences only to sentinel-shaped `0x0000`/`0x003D`/`0xFFFF` values, verified by direct static ROM dump). The remaining 3 (`$C3A4`, `$973A`, `$C5F3`) each open into a different piece of the game's own large, general, shared runtime machinery -- the per-frame active-actor-slot loop (`$C4E0`, needs an unfound spawn/placement table), the shared `$1F35` condition-dispatch jump table (needs a full catalog of its own handlers), and a distinct direction-based per-actor table search (needs an alignment resolution) respectively -- none of which is a small, cleanly enumerable data source the way `$26DC`'s was. Each is a legitimate, well-scoped, but substantially larger follow-on investigation in its own right, not a quick static check. The `$31AD` redirect mechanism itself remains fully reverse-engineered and live-confirmed (see the two entries above); what concretely, naturally produces a bank-14-reaching HL value in real play is not resolved this session despite exhausting every currently-known static tracing angle on all 15 real callers. No production `src/` code changed; `luajit tests/run_tests.lua` unaffected.

## 2026-08-19, direct follow-up ("dann versuche eine andere strategie! wir müssen das lösen und wir können das nicht live tracken"): SOLVED -- the real, in-corpus, natural trigger for a bank-14 `CutTransitionTable` record found, via this project's own static tooling, no live mGBA needed

Direct instruction to brainstorm and try genuinely different, non-live-tracking strategies. This pass tried, in order:

1. **Traced `$3213`'s own 3 real callers' HL setup** (never done before) -- found they route through the EXACT SAME `$26DC`/record-7 mechanism already ruled out for `$31AD`. Decisive, satisfying closure: this explains WHY the already-proven real entrance-seal trigger lands in bank 13 (record 7's grid values all have `H` outside `[0x80,0xBF]`, hitting `$3C4F`'s own bank-13 default) -- corroborates the whole mechanism is understood correctly while confirming (again) it can't reach bank 14. Also checked for indirect/table-driven callers of `$26DC` (raw byte search for `DC 26`) and for sibling "×11 multiply, index into the same bank-8 table" call sites elsewhere in the ROM -- both came back empty, `$26DC` really is unique with its one fixed record.
2. **Searched for callers of `$3282`/`$31C7`/`$3C4F` directly** (bypassing `$31AD`/`$3213` entirely) -- found `$3282` has 3 real callers, not 1: the already-known internal one from `$31EA`, plus two brand new ones, `$393B` and `$C892`.
3. **Disassembled `$393B`**: it's `$3937`, a leaf shared by 3 real script opcodes -- and this project's OWN `ScriptOpcodeTable.lua` already had these opcodes catalogued (`0xC9`/`0xCA`/`0xCB`, found at task #86, "$3937 leaf is untraced -- HONEST SCOPE"). Full disassembly closes that gap: these opcodes read a 2-byte BIG-ENDIAN operand straight from the script bytecode, call `$3282` with it directly, and write the resolved result into one of 3 independent WRAM slots (`$D613`/`$D623`/`$D633`) -- **a real "spawn an independent parallel sub-script" primitive, NOT a jump within the calling script** (confirmed live in step 5 below: the calling script keeps running its own next instructions completely undisturbed).
4. **Corrected a real search-methodology gap**: every earlier literal-value search this session used little-endian byte order; this opcode family's own documented convention is BIG-ENDIAN. Re-searched for the exact target (5744) in both orders -- still zero genuine hits (one big-endian raw match was checked and correctly diagnosed as a coincidental misalignment inside an unrelated real bank-7 pointer table, the same class of trap already caught earlier this session).
5. **Reused this project's OWN `ScriptRuntime`/`RomScriptStream`/`ScriptOpcodeTable` infrastructure (not live mGBA) to run the FULL real `scriptPointerTable` and capture every literal operand these 3 opcodes ever use, corpus-wide.** This surfaced a second, major, independent discovery along the way: **this project's own existing `scriptPointerTable` doc comment ("index 1357 onward is uniform 0xFFFF filler") is WRONG** -- real, non-filler data resumes almost immediately and continues in large real stretches all the way to the table's true end at index 6262 (see the matching correction now in `rom_profiles.lua`'s own doc comment). Scanning turned up 95 real `C9`/`CA`/`CB` occurrences across 48 distinct operand values; most are implausibly large (beyond the table's own real 6263-entry bound, meaning they'd read outside bank 8's mapped window on real hardware -- an honest, flagged, unresolved caveat) -- but one, **operand 715**, is well within range, matches the `$3C4F` bank-14 test, and traces to **script index 706 -- squarely within the ORIGINAL, independently-verified, trustworthy 1357-script range**, not one of the newly-found extra entries.
6. **Live-traced script 706's own real opcode sequence using the project's own interpreter (still no mGBA)**: `CB(715)` fires as its own literal, completely UNGATED first instruction, then the script continues normally through several more real opcodes -- direct proof this mechanism doesn't touch the calling script's own cursor, and that reaching `CB(715)` requires nothing more than script 706 itself ever running.
7. **Resolved the target**: table index 715 -> raw `0x4373` -> (via `$3C4F`'s own already-live-confirmed formula) bank 14, file `0x38373`. 2 bytes further in (file `0x38375`) is a fully recognizable, real `CutTransitionTable`-shaped 9-byte record (`00 05 F4 0F 33 0E 0C 00 0B`, `roomSelector=15`, the `unknownRoomB` family, matching this project's own already-catalogued real transitions exactly).

**Net result**: for the first time this session, a REAL, already-catalogued, trustworthy script (index 706, not a forced/injected value) unconditionally fires an instruction that resolves to a genuine bank-14 `CutTransitionTable` record -- found entirely through this project's own static tooling, no live emulation. This also reframes what `CutTransitionTable` most likely represents: real top-level SCRIPT content (part of the normal 1357-script corpus) triggering a transition as one of its own actions, not player-walked door/collision logic -- a strong structural explanation for why this session's own exhaustive live boundary-walking of every currently-reachable room found zero hidden doors anywhere.

**What remains open, now much better scoped**: what real game event ever selects script index 706 to run in the first place. This is a concrete, well-defined next question (which of the game's many known script-selection paths -- NPC dialogue, room-load, item use, etc. -- ever picks index 706), not "does a natural trigger exist" (now answered: yes). A second, honestly-flagged loose end: script 706 also fires the same opcode family with 3 further operands (16298, 16299, 10955) that exceed the table's own real bounds -- not explained this pass. No production `src/` code changed beyond doc comments (`ScriptOpcodeTable.lua`'s `0xC9`/`0xCA` entry, `rom_profiles.lua`'s `scriptPointerTable` entry); `luajit tests/run_tests.lua` unaffected. Scratch tooling (`scan_c9_ca_cb_full_table.lua`, `trace_script_706.lua`) kept in the scratchpad, not committed, per this project's own convention.

## 2026-08-19, same-day direct follow-up ("Skript 706 zu Ende verfolgen"): a real self-caught error in this session's own `$26DC` claim, reconciled against an earlier session's already-more-thorough investigation -- practical verdict UNCHANGED

While chasing what selects script index 706, cross-checked this session's own `$26DC` conclusion against `CutTransitionInterpreter.lua`'s doc comment (which references `$026DC` as a "roomSelector-argument subroutine" live-observed with `A=4`) and found a real, concrete self-caught error: **this session's own earlier claim that "all 5 real static callers of `$26DC` pass the identical literal `A=0x07`" is WRONG.** Re-reading this session's own disassembly dumps directly: only 3 of 5 (`$4261`, `$42A0`, `$BB81`) hardcode `A=7`; the other 2 (`$434F`, `$4395`) load `A` DYNAMICALLY, from WRAM `$D49D` and register `C` respectively -- a plain misreading of already-printed output, not a new fact.

**Re-investigated properly**: dumped bank-8 `roomSelectorTable`'s OTHER real bank-6 records (indices 1, 8, 10, 14, 15 -- the full set with dynamic-bank byte `6`, matching `CutTransitionInterpreter.lua`'s own documented `$C3F0=6` live observation for the boss-defeat trigger). Records 8, 10, 14, and 15 (not just 7) DO have real grid cells whose dereferenced value resolves to valid bank-14-reaching `scriptPointerTable` indices -- a real, decisive numerical correction to the blanket "record 7 is the only one that matters" framing.

**However, this does NOT reopen a new path** -- a MUCH earlier, more thorough session (`docs/reverse-engineering/rom-map.md`, predating this one) already fully traced BOTH dynamic-`A` call sites to their own real producers and reached a decisive, careful conclusion this session hadn't re-read: 3 of 5 hardcode the unrelated placeholder `roomSelector=7`; the `$434F` path only ever mirrors WHATEVER ROOM IS ALREADY ACTIVE (`$C3F5`, the live "current room" cell) -- it cannot introduce a new, not-yet-visited room; and the `$4395`/register-`C` path is "driven entirely by whichever script's own literal bytes are executing, not a separate selectable table." That session's own stated conclusion: **"the real, only remaining path... is: some specific real script... contains one of the 36 already-catalogued `unknownRoomA` landing-record byte sequences directly in its own body"** -- which is EXACTLY what this session's own script-706/opcode-`0xCB` finding turned out to be (a completely different, independent mechanism from `$26DC`, not something `$26DC` itself provides). The numerically-valid bank-14 grid cells found in records 8/10/14/15 this pass are real data, but not a live-reachable path on their own -- `$26DC` can only ever reflect an already-active room, and none of the currently-reachable rooms are 8/10/14/15's own family.

**Corrected, honest net effect**: the earlier "$26DC ruled out" verdict stands, now for the CORRECT, more complete reason (already established in `rom-map.md`, not freshly re-derived here) rather than the incomplete "only record 7 exists" framing this session had used. No change to the practical status of the room-connectivity investigation -- script 706 (via opcode `0xCB`, a genuinely separate mechanism) remains the one real, live, in-corpus trigger found. What selects script index 706 as a NEW top-level script (as opposed to `$31AD`'s own mid-script redirect, which is what index-232/boss-defeat uses) is a genuinely different, not-yet-investigated question -- this project has no existing documentation anywhere of a general "what selects a fresh top-level scriptPointerTable index" mechanism; this is real, new, uncharted ground for a future pass, not something already answered elsewhere and missed. No production `src/` code changed; `luajit tests/run_tests.lua` unaffected.

## 2026-08-19, same-day direct follow-up ("neue Tiefe: wie wird ein neues Top-Level-Skript ausgewählt jagen"): 4 static angles tried for "what selects script 706", all clean negatives

Direct continuation, chasing the newly-scoped "how does a fresh top-level script get selected" question (distinct from `$31AD`'s own mid-script redirect).

1. **Full-ROM scan of every real write site to `$D8B6`/`$D8B7`** (the persistent script cursor itself): exactly 9 pairs. 8 of 9 sit inside the already-fully-mapped `$31xx`-`$3Cxx` cluster (the `$31AD`/`$32D6`-queue/opcode-`0x00` family). The 9th (bank 2, file `0xA9AA`) is genuinely different -- but disassembly shows it's a "rewind the CURRENT cursor by 2 bytes" operation (`$D84A==0x1E` gated), not a fresh-script selector.
2. **Traced the `$32D6`/opcode-`0x00` WRAM queue** (`$D8BC`/`$D8BD`, the "4 halt paths, 1 continue-and-redirect path" mechanism documented earlier this session): exactly ONE real write site anywhere in the ROM (file `0xAEC4`-`0xAECC`, bank 2) -- and it writes a literal WRAM address (`$D683`), not a ROM script pointer. Reads as part of a large, general game-state-initialization routine (touches a wide spread of WRAM cells, `$D7Bx`-`$D7Dx`), redirecting to a WRAM-resident dynamic mini-script (the same `$D613`-family convention already documented for boss-defeat), not the `scriptPointerTable` mechanism at all.
3. **Literal-constant search**: neither `706` itself, its resolved CPU address (`$4306`), nor its raw table value (`0x0306`) as an `LD reg,imm16` appears anywhere hardcoded in the ROM (one raw-byte coincidence, `LD BC,0x0306`, checked and dismissed -- BC isn't the register this mechanism ever uses for indexing).
4. **Checked ALL 16 real `roomSelectorTable` records' own grids** (not just the bank-6 ones from the previous entry) for any cell whose dereferenced value, with the already-established `+0`/`+1`/`+2` correction, equals `706`: zero matches.

**Honest, complete conclusion for this pass**: 4 well-motivated static angles, all clean negatives. This does NOT mean no mechanism exists -- it means the answer isn't hiding in any of the places this project's own already-understood machinery (the persistent-cursor write sites, the WRAM queue, literal constants, the `roomSelectorTable` grid) would put it. The real answer likely lives in a genuinely different, not-yet-cracked subsystem this project hasn't mapped yet (an NPC placement/dialogue-trigger table, an item-use table, or similar) -- a real, open-ended search, not a bounded follow-up. No production `src/` code changed; `luajit tests/run_tests.lua` unaffected.
