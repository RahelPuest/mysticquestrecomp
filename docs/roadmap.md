...
| P2 | **NEW — Magic/spell system** | 🟢 which record is which spell + its real MP cost + the real ROM deduction code: CLOSED (2026-08-19); 🟡 **spell-effect DISPATCH ARCHITECTURE found same day** (2 of the real effect categories understood — LP-restore/Cure, status-cure/Heal); 🔴 elemental/attack spells' own category and a real spell-learning trigger still unfound | The 8 MP-costed castable spells (Cure/Heal/Sleep/Mute/Fire/Ice/Lightning/Nuke) turned out to be `ItemTable` records 0-7 (previously wrongly assumed "found/thrown combat items, never sold") — found by live-disassembling the real ROM's own MP-deduction routine (bank 2 CPU `$718F`-`$71AB`, resolves back to `categoryByte AND 0x1F` of the SAME already-decoded item table, byte 8), 8/8 external MP-cost matches. Same-day follow-up traced the real post-deduction dispatch: a per-effect-category table chain (bank 2 CPU `$7B29`/`$7B31`/`$7B38`, resolved via a shared `$7155` membership check) — Cure's heal amount uses the combat PRNG feeding the SAME level-indexed growth-curve routines (`$2B7B`/`$2B8B`) the character level-up formula already uses; Heal turned out to be a status-cure spell (4-bit-masked, not a second HP-heal), resolving the Cure-vs-Heal naming ambiguity. Website's item table (previously mislabeling isSpell exactly backwards) corrected. Still open: the elemental/attack spells' own effect category (not yet located), and how a fresh character ever learns a spell (live-verified via mGBA: the real menu's "Magie" option genuinely closes with zero effect for a character with no known spells — matches this project's own existing UI behavior). See events.md's own 2026-08-19 entries (2 dated passes). |
