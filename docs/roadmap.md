...
| P2 | **NEW — Magic/spell system** | 🟢 which record is which spell + its real MP cost + the real ROM deduction code: CLOSED (2026-08-19); 🟢 **full 8-category effect-dispatch chain mapped same day** (every real category table found and its full membership read from ROM); 🔴 Ice's own effect genuinely missing from every table (flagged, not resolved) and a real spell-learning trigger still unfound | The 8 MP-costed castable spells (Cure/Heal/Sleep/Mute/Fire/Ice/Lightning/Nuke) turned out to be `ItemTable` records 0-7 (previously wrongly assumed "found/thrown combat items, never sold") — found by live-disassembling the real ROM's own MP-deduction routine (bank 2 CPU `$718F`-`$71AB`). Same-day follow-up mapped the COMPLETE real post-deduction dispatch: 8 chained effect-category tables (`$7B29`→`$7B31`→`$7B38`→`$7B46`→`$7B3D`→`$7B40`→`$7B43`→`$7B48`), each a real null-terminated membership list read directly from ROM. Decisive design finding: items AND spells share this ONE dispatch (e.g. the Cure spell and the shop potions Lebe/S-Lebe/Magi/S-Magi/Elixier all funnel through the same LP-restore code). Self-caught correction: an earlier walkthrough-order guess had Sleep/Mute backwards (both cost 1 MP, indistinguishable by cost) — real table-membership evidence (Blok groups with shop item "Stille"=Silence, Ruhe groups with "Schlaf"=Sleep) resolves it the other way. Honest, explicitly flagged anomaly: spell index 6 (Ice) does not appear in ANY of the 8 tables — not forced to a guess. Still open: how a fresh character ever learns a spell (live-verified via mGBA: the real menu's "Magie" option genuinely closes with zero effect for a character with no known spells). See events.md's own 2026-08-19 entries (3 dated passes). |
