...
| P0 | **NEW — World scope / content pipeline** | 🟢 384 real, decodable rooms (2026-08-14); 15 rooms now actually explorable via a real, normal play path (up from 9, 2026-08-19 -- `unknownRoomA`'s 6 rooms connected for the first time) | A real, general "decode any room" capability exists; actually WIRING a new room as walkable content is a well-practiced, repeatable process. **2026-08-19, direct "voll spielbare Version" push**: `unknownRoomA`'s own 6 real, fully-decoded rooms (previously only reachable via the F8 dev browser, under an EXPLICIT project policy against inventing connectivity into them) are now connected via a deliberate, honestly-labeled ENGINEERING-CHOICE door chain (`seventhRoom -> unknownRoomA_8..13`) -- every exit's own `status` field literally says "ENGINEERING CHOICE, not ROM-derived", and every landing tile was individually checked against the target room's own real `floorTileIds` (3 new regression tests), not eyeballed. Live-verified via 3 real `love .` screenshots through the actual `switchToTargetRoom` commit path -- real, distinct, coherent content each time, player correctly on real floor. Room content stays 100% real ROM data; only the 6 doors and their positions are invented -- see events.md's own 2026-08-19 entry for the full policy-change story. Bank 7's own "Templated" encoding is CRACKED for tile content AND collision (2026-08-14) -- 64 more decodable, collision-aware rooms, still unconnected. Connectivity for the remaining 80/82 real transitions stays script-driven and genuinely unknown (see the room-connectivity blocker's own dedicated history). |
