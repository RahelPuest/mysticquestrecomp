...
| P2 | **NEW — Magic/spell system** | 🔴 castable spells still not started; 🟢 **item shop table now fully priced (2026-08-18)** | The 8 MP-costed castable spells (Fire/Ice/Lightning/Nuke/Cure/Heal/Sleep/Mute) remain genuinely unfound — a direct+strided byte search for their external MP-cost signature came back negative. `ItemTable`'s real recovery/status-cure item catalog got a real `price` field instead (8/8 external gold-cost matches) — a genuine, adjacent win, not the milestone itself. See events.md's own 2026-08-18 entry for concrete next angles (search opcodes referencing `$D7B6`/`$D7B8` curMP/maxMP directly). |
