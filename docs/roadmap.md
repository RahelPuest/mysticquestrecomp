...
|---|---|---|---|
| P0 | **3 — Map/room extraction** | 🟢 pipeline GENERALIZED (2026-08-12); **9 real, walkable rooms wired** -- back to this number after a same-day add-then-retract (2026-08-20) | willyRoom/secondRoom/thirdRoom/fourthRoom/fifthRoom/sixthRoom/seventhRoom, plus the 6 dev-only `unknownRoomA` content records (no connectivity). `seventhRoom` (2026-08-16, direct user report) is the first room wired WITHOUT a live gameplay capture -- picked from the already-decoded 384-room catalog by real collision-byte walkability instead, behind a real `requiresFlag`-gated exit (secondBoss defeat). **2026-08-20: `worldMapRoom_131`/`132` and `unknownRoomA_8..13` connectivity, briefly added the same day, was fully RETRACTED after a direct, blunt user correction** -- `worldMapRoom_131` is a structurally unrelated bank-5 grid record, not a real neighbor of `seventhRoom`'s own bank-6 position; the user pointed at the real world-map neighbor (bank-6 record 52) instead, not yet investigated. `eighthRoom`/`ninthRoom` (previously content-decoded, bank-5 records 236/237) were ALSO reported wrong and **deleted entirely** from `profile.graphics`, documented as misidentified. See the P0 world-scope row below for the full retraction trace. |
| P0 | **NEW — World scope / content pipeline** | 🟢 384 real, decodable rooms (2026-08-14); 9 rooms actually explorable via a real, normal play path -- an attempted 2026-08-19 push to 15 (via `unknownRoomA`'s 6 rooms) was built, live-"verified", reported done, then RETRACTED same day after a direct user report | A real, general "decode any room" capability exists; actually WIRING a new room as walkable content is a well-practiced, repeatable process. **2026-08-19, direct "voll spielbare Version" push, then retracted same day**: `unknownRoomA`'s own 6 real, fully-decoded rooms (previously only reachable via the F8 dev browser) were briefly connected via a deliberate, honestly-labeled ENGINEERING-CHOICE door chain (`seventhRoom -> unknownRoomA_8..13`). It was pulled back after a direct, blunt user report ("alles nach dem 7. raum ist müll") triggered a full re-audit: the `floorTileIds` every landing spot depended on mismatched this project's own stated collision-byte classification rule for 42 of 82 tiles actually used, and even a fully mechanical, rule-consistent rebuild produced a checkerboard artifact with almost no real 2x2-footprint-walkable area in most of the 6 rooms (2 of 6 had zero connected walkable footprint at all). The original single-cell landing check never caught this because it didn't test the real footprint `TileWalkability.build` actually uses, and no in-game walk test happened before the feature was reported done -- both gaps now recorded as a concrete process lesson, see events.md's own 2026-08-19 retraction entry. Room content stays 100% real ROM data, untouched; only the invented doors were withdrawn, back to none, until a trustworthy floor source exists for this metatile table. **Same day, direct follow-up ("können wir jetzt nicht alle zusammenhängenden räume entschlüsseln?")**: a systematic edge-match scan of the FULL bank5 (16x16) / bank6 (8x8) world-map grids (not just the 2026-08-14 spot-check) found 6 record pairs with a 100% tile-exact shared edge -- real, objective evidence of genuine spatial adjacency. Of those, `worldMapRoom_131`<->`worldMapRoom_132` (bank5 records 131/132) is the one pair with a uniformly walkable, footprint-verified, non-fragmented, visually-confirmed shared edge -- wired as a real, honestly-labeled STRUCTURALLY-DERIVED (not live-trigger-confirmed) bidirectional door, live-verified via actual scripted walking through it in BOTH directions (not just a landing-spot check -- the exact gap that let `unknownRoomA` ship broken). Deliberately NOT yet connected to any normally-reachable room -- reachable today only via the same dev-only teleport machinery (`MYSTICQUEST_VICTORY_START_ROOM`) used to verify every other room this session; whether to add a real entry point from existing gameplay is a separate, not-yet-made decision. The other 5 perfect-match pairs were left unwired (their shared edges are uniformly WALL under the current, still-unverified collision rule for this metatile table -- see rom_profiles.lua's own doc comment for why that's not necessarily conclusive either way). Bank 7's own "Templated" encoding is CRACKED for tile content AND collision (2026-08-14) -- 64 more decodable, collision-aware rooms, still unconnected. Connectivity for the remaining 80/82 real transitions stays script-driven and genuinely unknown (see the room-connectivity blocker's own dedicated history). **Same day, direct "p0" follow-up**: ran a decisive, clean live flood-fill (fresh mGBA state reload per boundary probe, zero cross-contamination) of `startRoom`'s and `fourthRoom`'s own full real boundaries -- both fully enclosed, zero hidden passages toward the 2026-08-18 predicted castle-cluster neighbors via ordinary walking, including `fourthRoom`'s previously-untested east wall. One promising-looking anomaly (a real SCY scroll revealing castle-tower art) was checked against the real ROM identity register and correctly ruled out as stale leftover VRAM content, not a new room -- reported as a ruled-out false alarm, not oversold. **Same day, "p0 weiter"**: extending to `fifthRoom` surfaced a real, self-caught bug -- every flood-fill script this session had `$C244`/`$C245` (Y/X) backwards versus this project's own already-established 2026-08-09 finding, meaning the "boundary cells" tested for `startRoom`/`fourthRoom` were transposed rather than the intended ones. Verified the correct mapping empirically (single-axis poke + screenshot) before trusting either the old code or the old doc comment, fixed the script, and re-ran all 3 rooms correctly. Conclusions held (all 3 fully enclosed) but were previously right for the wrong reason -- recorded openly rather than left standing uncorrected. **Same day, "wieter mit p0"**: extended the corrected flood-fill to `willyRoom`/`secondRoom`/`thirdRoom` (the scroll-family, all sharing one real ROM identity `$46B0`) -- **this completes boundary-testing for EVERY currently-live-mGBA-reachable room in the game (6/6)**, zero hidden exits found anywhere; 2 of `secondRoom`'s own anomalies turned out to be a nice independent re-confirmation of the already-known real `secondRoom`->`thirdRoom` scroll (real `SCX=160` signature matched exactly), not a new lead. `seventhRoom` remains the one genuinely untested room (no live checkpoint exists to reach it at all) -- the walking-boundary method is now exhausted for the rest of the reachable world; further P0 progress needs either `seventhRoom`'s own real trigger or more progress on the separately-explored bank-14 script-transition investigation. **Same day, direct follow-up mapping the non-reachable world too**: extended the tile-exact edge-match scan to the FULL 384-room catalog, including bank 7 (never tested before) -- bank 7 shows a decisive 0.0% edge-match rate, real evidence it's a flat catalog, not a spatial grid like bank5/6. Bank5/6 re-confirmed complete via union-find: exactly 4 real connected-component clusters exist catalog-wide, only 1 (`131`/`132`, already wired) has a walkable shared edge -- the other 3 are uniformly wall. This fully closes "how are the non-reachable rooms connected" for this technique; no further clusters remain to find this way. **Same day, direct follow-up ("man du kennst doch die raum wechsel skripts. such die doch einfach")**: the bank-13/bank-14 `$31AD` selection mechanism (`$31AD` -> `$3282` -> `$3C4F`) is now FULLY reverse-engineered, including a real, self-caught correction that the table index is passed via HL, not BC as previously documented. Dumped the real bank-8 lookup table (file `0x20F11`) and found the single table index (5744) whose value exactly matches a real, known `CutTransitionTable` record (file `0x385FE`, roomSelector 2, pixel (136,112)). Live-injected a properly-simulated `CALL $31AD` with the corrected `HL=5744` -- `$D86A` (bank cell) and the persistent cursor updated to EXACTLY the predicted bank-14 values, a full, decisive confirmation of the redirect math and the first concrete input value this project has ever computed that provably drives the real mechanism into a real bank-14 transition record. Honest limit, checked directly: 1,000,000 further instructions produced no visible room change -- identity/position never updated, and the cursor drifted to an idle WRAM sentinel within ~20,000 instructions, consistent with a bare `CALL` simulation lacking the real frame/interrupt timing a genuine trigger would supply. What naturally sets `HL=5744` (which of `$31AD`'s 15 real callers, under what condition) remains unidentified. See events.md's own same-day entry for the full trace; `rom_profiles.lua`'s `entranceSeal` doc comment carries the technical writeup. **Same day, direct follow-up ("dann versuche eine andere strategie... wir können das nicht live tracken")**: statically (no live emulation) backward-traced ALL 15 real `$31AD` callers' own HL setup, closing this thread out completely. 8 of 15 use small literals (can never reach bank 14). Of the other 7: 4 (`$173F`/`$247C`/`$24A0`/`$24CD`, sharing one entry point `$26DC`) are now DEFINITIVELY RULED OUT -- their one real bank-8 record's grid dereferences only to sentinel-shaped values (`0x0000`/`0x003D`/`0xFFFF`), dumped directly and completely from ROM. The remaining 3 (`$C3A4`, `$973A`, `$C5F3`) each open into a different piece of the game's own large, general, shared runtime machinery (the per-frame active-actor-slot loop needing an unfound spawn/placement table; the shared `$1F35` condition-dispatch jump table needing a full catalog of its own handlers; a distinct direction-based per-actor table search needing an alignment resolution) -- each a legitimate, substantially larger follow-on investigation, not a quick static check. The redirect mechanism itself stays fully proven; the concrete natural trigger is not resolved despite exhausting every currently-known static angle on all 15 real callers. **Same day, direct follow-up ("dann versuche eine andere strategie... wir können das nicht live tracken") — SOLVED**: a genuinely different, purely-static approach (this project's OWN `ScriptRuntime`/`RomScriptStream` tooling, no live mGBA) found the real, natural, in-corpus trigger. Script opcodes `0xC9`/`0xCA`/`0xCB` (already catalogued, "leaf untraced") turn out to read a literal operand straight from script bytecode and spawn an INDEPENDENT parallel sub-script via the same `$3282` table lookup -- not a jump within the calling script. Scanning the full real `scriptPointerTable` (along the way, found and corrected another real doc error: entries past index 1357 are NOT uniformly filler as previously documented -- real data continues to index 6262) found script index **706** (within the original, trustworthy 1357-script range) firing `CB(715)` as its own unconditional, ungated first instruction -- resolving to a real, recognizable bank-14 `CutTransitionTable` record. First real, already-catalogued, non-forced trigger found this session. Reframes `CutTransitionTable` as cutscene/event-triggered script content, not player-walked door logic -- consistent with the exhaustive live boundary-walking finding zero hidden doors. Open follow-up: what real game event selects script 706 to run at all. See events.md's same-day entry for the full chain. **Same day, direct follow-up ("dann versuche eine andere strategie... wir können das nicht live tracken", repeated 2x more)**: corrected a real self-caught error in this session's own `$26DC` claim (2 of 5 callers load `A` dynamically, not all 5 hardcoding `A=7`) -- reconciled against a much earlier, more thorough session's own investigation (`rom-map.md`), which already reached the same practical "ruled out" verdict for the right reason and correctly predicted script 706's own nature. 4 further static angles for "what selects script 706" (persistent-cursor write sites, the `$D8BC`/`$D8BD` WRAM queue, literal-constant search, `roomSelectorTable` grid dereference) all came back clean negatives. **Then found a much stronger candidate**: `StandardScriptHandlers.chain()` (opcode `0x02`, CHAIN) uses the SAME `$3C4F` bank-13/14 correction `$31AD` does -- scanning the ORIGINAL trusted 1357-script range found 24 real CHAIN dispatches across 22 distinct scripts resolving to bank 14, 6 landing near already-catalogued real landing records for ALREADY-REACHABLE room families (`roomSelector` 1 and 3), matching the same mechanism `BossSequenceInterpreter.lua` already LIVE-CONFIRMED reaches bank 14 via CHAIN for the boss-defeat scene. A precise fresh disassembly of `$32FE`/`$3C4F` directly conflicts with `chain_cross_bank_test.lua`'s own existing "stays in the same bank" assumption for a similar case -- flagged as a genuine, unresolved tension (not silently fixed either way), since this exact class of question has, every other time this project has hit it, only ever been conclusively resolved via live verification. See events.md's own matching entries for the full chain. **Same day, direct follow-up ("ja, jetzt gezielt live prüfen")**: the "22 distinct scripts" claim above was WRONG -- the FIRST live check attempted (script 307, the highest-confidence candidate) flatly falsified it: `$3C4F` correctly selected bank 13, not 14, live-confirming the SCAN's own discriminator had a real bug (it checked the post-correction target's own high byte, which is genuinely ambiguous between "really corrected" and "never needed correction, just naturally landed in the same range" -- the raw pre-`+0x4000` operand bytes are needed to tell them apart). Re-scanned correctly: of the original 22, **exactly 1 survives** -- script index **495**. Live-puppeteered THAT one directly (same technique as the original `$31AD`/index-5744 breakthrough): `$D86A` becomes `0x0E`, the emulator's own real MBC bank state changes from 1 to 14, cursor lands at `$5E12` -- a full, clean, decisive live match. This project now has a SECOND, independent, fully live-confirmed real trigger for bank 14 -- entirely natural (a real CHAIN inside a real, already-catalogued script's own bytecode), though what selects script 495 to run as a top-level script in the first place remains the same open question script 706 also has. A genuine, textbook self-caught-and-corrected error: confident static claim (22), falsified by the first live check, root-caused, corrected (1), and that 1 immediately fully confirmed. See events.md's own matching entry for the full trace. **Same day, direct follow-up ("p0 hat prio, wie da weiter")**: pivoted back to `unknownRoomA`'s own SEPARATE blocker (broken floor data, not a missing trigger). Re-derived its collision data with the position-aware, per-metatile-instance technique already trustworthy for `worldMapRoom_131`/`132` instead of the flawed flat, tile-ID-keyed set -- found only 3-4 clean collision-byte values (not "many exotic values"), and large, healthy, non-checkerboard connected footprints for all 6 rooms under at least one of 2 already-known polarity rules. Real, positive progress (rules out "fundamentally broken data"), but inconclusive (no single polarity rule wins uniformly across all 6 records) -- `UNKNOWN_ROOM_A_FLOOR_TILE_IDS` left unchanged, connectivity stays withdrawn (still no live trigger to verify walkability either way). See events.md's own matching entry. **Same day, direct follow-up (visual verification instead of more byte-value guessing)**: rendered all 6 rooms with a collision-value color overlay and looked directly at the art -- DECISIVELY confirms the already-default rule (fourthRoom's convention) is correct here too (collision `0x30` = real brick wall texture, `0x00`/`0x08` = real mesh/net floor texture); the earlier footprint numbers favoring willyRoom's rule for 2 rooms was a red herring (those rooms are just smaller/more wall-dense by design). But found a DEEPER, real blocker underneath: 4 of 82 tile IDs genuinely conflict (real floor in most occurrences, wall-adjacent decoration in a few), and excluding them conservatively collapses connectivity to 2-5 cells in 5 of 6 rooms through the actual production pipeline -- a genuine DATA-MODEL limitation (flat tile-ID-keyed `floorTileIds` cannot represent this table), not a wrong-rule problem. Needs real position-aware collision storage in the rendering/walkability pipeline -- a structural change, deliberately not attempted without checking in first. **Same day, direct follow-up ("p0 hat prio, wie da weiter") -- CLOSED**: the "needs a structural change" conclusion was smaller than expected -- the mechanism (`TileWalkability.buildFromCollisionGrid`, position-aware, no flat set) already existed and `RoomExplorer.lua` (F8) already used it for its general 384-room browsing; it just had a real regression (bank-5 records 8-13 decoded via the generic catalog metatile table/tileset instead of `unknownRoomA`'s own dedicated, verified pair). Fixed, live-verified via 2 real screenshots (`MYSTICQUEST_DEBUG_STATE=roomexplorer:9`/`:14`) matching `render_unknown_room_a.py`'s own independent rendering exactly. F8 now shows `unknownRoomA` with real art AND real position-aware walkability -- the "trustworthy floor source" gap is closed. Connectivity into normal gameplay stays withdrawn either way (still no live trigger). See events.md's own matching entry. **Same day/next day, direct follow-up ("p0 hat prio, wie da weiter", live check approved)**: cross-referenced all 38 distinct `roomSelector` 1-6 `CutTransitionTable` landings (already-reachable `startRoom`/`fourthRoom`/`willyRoom` families) against every already-wired exit -- found 2 high-occurrence, unwired candidates (`(80,72)`, `(136,112)`) reused across `roomSelector` 2-6. Live-tested (axis-precise walk + `A`-button) at 6 of 8 real room/target combinations, landing EXACTLY on target every time: clean, decisive negative -- neither walking through nor pressing `A` triggers anything. Rules out the simplest mechanisms; a script/story-gated trigger (matching this session's other bank-14 findings) remains untested. **2026-08-20, direct user priority ("Räume anschließen") -- the bank-13/14 script-trigger thread above reached a genuine plateau (fully reverse-engineered mechanism, zero new reachable content) and was deliberately set aside in favor of the higher-leverage, lower-risk move**: connected `worldMapRoom_131`/`132` (footprint-verified since 2026-08-19, but dev-shortcut-only) to the real reachable graph via a new bidirectional `seventhRoom<->worldMapRoom_131` door (real east/west edges, empirically confirmed reachable by BFS over the SAME `Player:update`/`TileWalkability.build` code the game itself runs, not a hand-derived row/col formula -- a first hand-picked attempt was caught as unreachable by this exact technique before any live run). **Live-confirmed in BOTH directions via real `love .` launches** (scripted input + state-log readback, not just a landing-spot check): `seventhRoom->131` lands at the exact configured `(16,88)`; `131->seventhRoom` lands at the exact configured `(128,88)`. 11 rooms now reachable via normal play (up from 9), the first net reachable-room gain since `seventhRoom` itself. `luajit tests/run_tests.lua`: 588/588. See events.md's own matching 2026-08-20 entry for the full trace, including a real methodology find (`MYSTICQUEST_VICTORY_START_ROOM`'s `pairs()`-order ambiguity once a room gets 2 real entrances). **Same day, direct user directive ("die gesamte Konnektivität aller Räume entschlüsseln und in die App und der Website einbauen")**: re-scoped honestly first -- structural/edge-match connectivity across the FULL bank5/6/7 catalog is already exhaustively closed (6 real pairs total, 1 walkable; bank 7 confirmed a flat catalog, 0.0% edge-match), and the deep script-trigger mechanism (`$31AD`/CutTransitionTable) stays exactly where the prior investigation left it -- neither reopened without a genuinely new angle, none found. Instead, applied the SAME `worldMapRoom_131`-style recipe to `unknownRoomA_8..13`: this family's own 2026-08-19 broken-floor-data retraction no longer applies (F8/RoomExplorer already got real, position-aware collision for it) -- gave `VictorySequence.lua` the same per-room collision special-case (mirroring `willyRoom`'s own pattern), re-checked adjacency PROPERLY this time (against the room family's own dedicated metatile table, not the generic one the original whole-grid scan used) and found a decisive negative (0/16 rows match for every consecutive pair) -- so wired all 6 as independent spokes off `worldMapRoom_132` (the hub) instead of a chain, zero structural-adjacency claim, honestly ENGINEERING CHOICE. **3 real bugs caught before any live run** by simulating the exact production movement code first: a landing/zone coordinate mix-up in the new tests themselves, a real zone collision with the pre-existing exit-to-131 zone, and a subtler one only found by simulating a real landing+idle-tick sequence (`Player:update`'s own unconditional bounds clamp silently relocates the pre-existing `worldMapRoom_131` landing spot onto a naive first version of `unknownRoomA_12`'s own new zone). **Live-confirmed round-trip for 3 of 6 rooms** (`unknownRoomA_8`/`_9`/`_13` -- the roomiest, a mid-constrained single-edge case, and the most constrained) via real `love .` launches, exact coordinate matches every time; the other 3 rely on the same BFS-over-real-`canMoveTo` evidence, not individually live-GUI-tested, stated as a real, lower confidence tier rather than implied equal. **17 rooms now reachable via normal play (up from 11)**. `luajit tests/run_tests.lua`: 598/598. `eighthRoom`/`ninthRoom` (already linked to each other via a real edge match) investigated as the next candidate, found genuinely fragile (a very sparse, oddly-shaped room -- confirmed no data bug, just little room for a second safe entrance) and deliberately deferred rather than rushed. Bank 7's 64 rooms and the ~300 other generic-catalog records have zero structural connectivity evidence and would each need this same per-room rigor -- no bulk shortcut exists without fabricating unfounded connections. **RETRACTED, same day, direct blunt user correction** ("die ganzen neuen Räume sind... die Grafiken da völlig falsch... die Transitionen völlig für den Arsch... vom Seventh Room in den World Map Room 131 ist völliger Quatsch. Das geht da nicht lang. Schau dir die World Map an, vom Seventh Room rechts daneben auf der World Map ist der richtige Room... Mach die 131 raus, mach die 132 raus und mach Unknown Room 8 bis 13 komplett raus... Auch Eighth Room und Ninth Room, falsch. Lösch sie komplett aus deinen Unterlagen und schreib, dass die falsch erkannt wurden."): BOTH connectivity additions from earlier this same day are fully withdrawn, taken at face value with no re-litigation. `worldMapRoom_131` (bank-5, 16x16 grid) and `seventhRoom` (bank-6, 8x8 grid) are structurally unrelated grids -- the door never had real adjacency evidence (always labeled "zero structural evidence" from the start); the user pointed at the real world-map neighbor directly (bank-6 record 52, immediately right of `seventhRoom`'s own position) -- not investigated or wired this pass, a genuine lead for later. `seventhRoom`, `worldMapRoom_131`, `worldMapRoom_132`, and all 6 `unknownRoomA` rooms are back to zero exits. `eighthRoom`/`ninthRoom` were separately, explicitly reported wrong and **deleted entirely** from `profile.graphics` -- documented as misidentified, not merely disconnected. Room count is back to **9** reachable rooms, matching the state before this whole day's work. `luajit tests/run_tests.lua`: 580/580 (18 tests removed along with the retracted connectivity, 2 corrected for the new `eighthRoom`/`ninthRoom`-free room/tile counts). See events.md's own 2026-08-20 retraction entry for the complete record. |
| P1 | **7 — Script/event system** | 🟢 **all 256 opcode values now classified (2026-08-16)** — 201 decoded/wired, 49 confirmed no-op, 6 known-hard by design, **0 genuinely undecoded** (up from 187/256 covered, ~50 still-open addresses); whole-corpus scan `clean` at 883/1357 (up from 871). The real interpreter now also drives 2 real transitions' room selection live in actual gameplay (2026-08-16, `thirdRoom→fourthRoom` and `fourthRoom→fifthRoom`, see below), not just a shadow run | 6+ census rounds plus a new whole-corpus scan tool (shadow-runs all 1357 real scripts, not just one) found and wired most of the actor-flag/queued-action/trigger-event/actorSlotPosition/actorAction-family opcodes; a `ScriptRuntime` actually RUNS real, decoded scripts against live ROM bytes (behind `MYSTICQUEST_SCRIPT_INTERPRETER=1`, reported via the debug overlay), and (2026-08-13/14, task #84) has driven its first real, VISIBLE output. Still parallel to, not replacing, `Field.lua`'s hand-authored `FIELD_EVENTS`/`VictorySequence` room-graph for anything beyond that one live-wired transition. "0 undecoded" means every opcode VALUE has a real, verified classification (decoded/no-op/known-hard) — it does NOT mean every opcode has gameplay-ready behavior: the 6 known-hard ones are traced but deliberately left unwired. UPDATE 2026-08-19: the 2026-08-16 "bank-call-stack eviction" diagnosis turned out to rest on an injection technique that never actually worked -- it poked mGBA's own cached `currentBank` display field, which a direct test this pass proved has ZERO effect on real memory reads or CPU fetches. The CORRECT primitive (a real `$2100` bus write, verified byte-for-byte) plus a correctly-disciplined stack push (fully disassembling `$29FB`/`$2A0A` for the first time) gets meaningfully further: the first bank-stack pop now correctly restores the real, pre-existing organic bank rather than an arbitrary default, disproving "random concurrent churn" as the root cause. Still doesn't reach `0xBA`'s real handler though -- SAME-DAY FOLLOW-UP found the precise reason: the real dispatch cycle unwinds 3 nested `RET`s (using the genuine, untouched Z80 `SP`) before reaching code that deliberately relocates the stack (`LD SP,HL`) and then `RET`s from it -- landing in WRAM's echo-RAM mirror because `HL`'s value at that exact point was never under our control. FURTHER SAME-DAY FOLLOW-UP tested a "filter by bank-stack depth" fix empirically (3,000,000-step real scan) -- REVISED: depth is constant (194) across all 39 real `$3727` hits observed, not a usable signal, and all 11 distinct real callers found share the identical `CALL $3727`/`RET` shape -- ordinary opcode-dispatch tails, not "unrelated subsystems" as the previous entry framed it. The real gap is narrower and sharper still: `$49AC` isn't `0xBA`'s own documented handler (`$0EB2`) at all -- the opcode-table LOOKUP machinery itself computed the wrong target, meaning it depends on more real input than bank+HL alone. **BREAKTHROUGH, SAME DAY, 5th round**: studying a clean (non-injected) reference trace of the identical natural moment found the real remaining bug -- the "disciplined push" adds an EXTRA stack level, shifting the real subsequent pop UP by one. Fix: overwrite the EXISTING top slot in place instead of pushing a new one. With that + the already-fixed bus-write bank switch + starting the injection from an ALREADY-ACTIVE real script context (not an arbitrary early-boot moment) -- **live execution reached the real `0xBA` handler (`$0EB2`) for the first time in this project's history**, and ran it: confirmed byte-for-byte against `ScriptOpcodeTable.lua`'s own existing static analysis (a real entity-slot allocation via `$0A74`, a genuine 20-slot live scan, populating a real slot with real data). The methodology is now general and reusable -- a working live-injection primitive (verified bank switch + verified stack overwrite + verified starting context) that can be pointed at the other 5 known-hard opcodes or any other unreached script. Not yet wired into the Lua interpreter, and the full entity-struct field semantics + independent confirmation of the despawn half aren't fully closed -- honest, well-scoped follow-up. Five real rounds deep into this thread, each honestly reported, the last one a genuine, historic payoff. See events.md's own 2026-08-19 entries (4 dated passes) for the full trace. **Same day, direct follow-up on `battleIntro.entranceSeal`'s own close mechanism**: traced it to a real script invocation of the already-decoded opcode `0xB0` (`byteWordCommand`, `$0F1E`), and found it's dispatched via a SECOND, independent, live-observed real trigger of the already-known `$31AD` "cross-actor redirect" mechanism (task #149, previously only seen for the boss-defeat sequence) -- decisive proof `$31AD` is genuinely general, not a one-off, and that it demonstrably reaches bank 13 in real play, one bank past the exhaustive static simulation's own ceiling (bank 12). Does not itself reach bank 14 (the real remaining connectivity target) -- what calls `$31AD` for this specific trigger wasn't traced further -- but narrows the bank-14 search to "find `$31AD`'s other real callers," a much smaller target than the whole 1357-script corpus. **UPDATE, same day, "weiter" x3**: found and checked all 7 of `$31AD`'s own real callers with any promise. Along the way, self-caught a real scanning bug (every prior static scan this session only checked `CALL`/unconditional `JP`/conditional `CALL` opcodes, never conditional `JP`) -- fixed, re-verified every earlier "zero hits" conclusion still holds except one. Result: 4 of 7 candidates got a real, precise, or live-testable outcome (one, `$8EDD`, resolved to an exact condition -- WRAM `$D84A==0x11` -- then live-tested clean across every currently-reachable checkpoint), all honest negatives; the other 3 resist static tracing entirely (indirect/computed dispatch, not found this pass). None of the 7 fired for currently-reachable content -- the bank-14 search via this specific angle is now honestly exhausted for now, consistent with the session's repeated finding that real further content sits past the current reachable frontier. See events.md's own dated entries for the full trace. |
| P1 | **9 — Combat (remainder)** | 🟢 both directions' real formulas now understood (2026-08-16); weapon-power question now DECISIVELY, empirically CLOSED negative (2026-08-16, same day) | Real per-species ATK fully extracted (11 species); player-attacks-enemy MAJOR CORRECTION 2026-08-16 -- real PRNG-driven formula found (same shape/PRNG as `$50AC`), the earlier "flat, no formula" read was a floor-rounding coincidence at the only testable base value, see combat.md. Live WRAM-poke experiment (`$D6E9`, the equip-slot cell `$5BA7` already, decisively reads) found the damage-formula base is unaffected by any of the other 5 real class/kit index values -- byte-exact identical combat outcome across all 5, and even `$D6C1`'s own attack-stat readout doesn't react to the poke. Working conclusion: the base (`4`) is most plausibly a fixed per-attack-type constant, not per-weapon power -- see combat.md's own dated entry. **2026-08-19, direct follow-up (pivoting off the P0 connectivity thread to close a real, already-decoded gap)**: the real, PRNG-driven player-attacks-enemy formula (found 2026-08-16 above) was never actually WIRED into gameplay -- `Enemy:hit()` still applied the flat `PLAYER_ATTACK_DAMAGE=4` constant directly, unlike contact damage (already wired to `CombatFormulas.rollDamage` the same day). Added `CombatFormulas.rollPlayerAttackDamage(base, noiseByte)` (a genuinely separate function, not a call to `.rollDamage` with a fabricated ATK/DEF pair, since the real chain has no enemy-side DEF term at all) and wired `Enemy:hit(noiseByte)` / `Field.lua`'s attack-hit call site to it, mirroring the contact-damage wiring exactly (same shared `combatNoise` PRNG stream, same `nil`-safe fallback for love-free/no-ROM contexts). Changes nothing observable today (base=4's own noise contribution mathematically floors to 0 for every possible byte, proven and tested) -- matters the moment a second weapon/enemy is reached. 3 new tests added (`combat_formulas_test.lua` x2, `enemy_test.lua` x1); `luajit tests/run_tests.lua`: 587/587 passing. |
| P1 | **NEW — Bestiary (multiple enemy types)** | 🟢 SOLVED end to end (2026-08-20) — a second, real, fightable creature (real captured sprite, independent HP/combat) now stands in live `Field.lua` gameplay, verified via a real LÖVE boot against the real ROM | `EnemySpeciesTable.lua` has real ATK for all 11 real species — ready for wiring once P0's room work surfaces a real spawn trigger for any of the other 10 (this project does not fabricate a species-to-room mapping without ROM evidence). **2026-08-18: shares its exact root blocker with Level/XP below** — `EnemyStatTable.lua`'s own `xp`/`gold` fields are real and non-zero for 15 of 21 species; reaching a second enemy unblocks both at once. **2026-08-19, direct "p1" follow-up**: `MonsterDefinitionTable`'s own documented "HP written straight to WRAM" claim CORRECTED — real disassembly shows it's a live multiply-then-shift formula (`$2B7B`, a real 8x8->16 routine) against a caller-supplied factor not stored in the table, not a raw copy (fixed before this caused a second live-facing wrong claim; none existed yet). Separately, rendered all 21 real `MonsterDefinitionTable` rows' actual sprite art (same pipeline already validated for the known boss) — confirmed real, visually distinct monster designs exist for at least 6 of the other 20 rows, consistent with `EnemySpeciesTable`'s own 11 species. Real, additive progress, but does NOT unblock Bestiary by itself — the root blocker is unchanged: no known spawn trigger reaches any of them yet. **2026-08-20**: re-checked the "reach a second enemy" blocker directly before assuming it was fresh territory — found EVERY currently-reachable checkpoint has already been exhaustively live-traced across many prior sessions (entity-table diffing during movement in 3 rooms: zero new spawns; the script interpreter confirmed NOT running at all during idle overworld exploration). A genuinely new lead was still found: a full message-settings-table re-census (correcting an old "1357 records" mix-up — the real table has 605) surfaced a SECOND boss-species cluster (`0x14`, Dragon family) never shadow-run before, with a real trigger script (index 427) no prior session had found. Live-tested with this project's own working puppeteering primitive (same bank-stack fix that first reached opcode `0xBA`) — reached the exact same real conditions as that success (depth 194, step 4919), but redirecting the cursor to a genuinely UNRELATED script (unlike `0xBA`, organically reached within the boss-defeat script's own flow) failed within 22 steps: a clean, decisive negative that precisely confirms the same real limit the `$31AD`/index-5744 forced-call experiment already found. Root blocker unchanged, but now understood more precisely: this project's live-injection method can reach already-active code, not force an unrelated script to become the top-level cursor. See events.md's own 2026-08-20 entry. **2026-08-20, same-day "frame/interrupt investment" follow-up**: traced the real per-opcode dispatch (`$3260`-`$3273`) end-to-end and confirmed via `CallTracer` (call depth 0 at every hit, 5/5 checked) that it is NOT interrupt-driven — it runs top-level in the main loop, at a measured ~1300-1350-CPU-step cadence. While tracing backward, found a SEPARATE, previously-undocumented real subsystem: an 8-slot live entity-instance table at WRAM `$C4E0` (24-byte stride, matching `EnemyStatTable`'s own record shape, `0xFF`-first-byte = empty sentinel — live-confirmed empty post-boss-defeat), scanned once per frame via `$C5A0`/`$CB4F`/`$4B19`/`$429B`, each active entry driving its own independently-cooled-down update via `$404A`. **2026-08-20, same-day "ja" follow-up — tested and corrected**: live-checked `$C4E0`/`$C5A0` during the real courtyard boss fight itself first — stayed completely empty the whole fight, directly disproving the "promising target" framing above. Tried the live write anyway (approved): the synthetic record WAS genuinely picked up by the real per-frame engine loop (cooldown/reload logic matched the disassembly exactly), but it triggered the game's own INTRO narration script ("AAAA ist ein tapfer...", not a monster), then visibly corrupted follow-on state — the same instability class as the script-427 negative. Honest correction: `$C4E0`'s `record[0]` is a general event/script-slot ID, not a species ID; this is not the second-enemy mechanism after all. Root blocker unchanged, genuinely re-opened rather than assumed. See events.md's own third 2026-08-20 entry. **2026-08-20, same-day "sei kreativ" follow-up — SOLVED**: direct user request for genuinely new strategies. Mined the external FFA-Disassembly (US cartridge) for its ENCOUNTER/spawn code specifically (not just boss stats, the only thing it had been used for before) and found a real `NPCSpawnPointers` table (109 rows x 3 cols, CPU `$7142`) driven by 2 real script opcodes, `sSET_NPC_TYPES`/`sSPAWN_NPC` — which turned out to be THIS project's own already-traced (but unidentified) opcodes `0xFC`/`0xFD`. Cross-confirmed byte-for-byte identical at `$7142` in this EU ROM too. A raw whole-ROM CALL census for the entity-alloc primitive (`$0A74`) found exactly 7 real call sites total, closing the chain end to end. Live exploration (a per-instruction-driven replica of the existing checkpoint chain, since bulk `Session.run()` has no per-opcode visibility) caught 2 real `0xFC`/`0xFD` firings neither prior corpus-wide scan had found (both resolve to friendly NPCs — Willy, Amanda). Built a 2-byte-patched ROM copy (scratchpad only) redirecting one of those REAL, already-firing triggers from `NPC_WILLY` to `NPC_GOBLIN` — the full chain fired live (`$27F9`->`$2820`->`$42BD`->`$0A74`, entity-alloc actually called) and **a second, visually distinct creature now stands on screen next to the player**, stable across frames, with real LP loss from contact. Same winning pattern as the `0xBA` breakthrough (data-only edit inside an already-organically-executing real path), opposite of the script-427/`$C4E0` attempts that tried to force a foreign entry point. Root blocker resolved at the ROM-mechanism level; **2026-08-20, same-day "ok bau die lua reimplementation" follow-up**: built it. Found and fixed the real `ctx.onTriggerEvent` naming collision at its actual root cause — the EXACT same precedent bug this project has already fixed 4 times in `ScriptRuntime:registerStandardHandlers()` (a later generic sweep silently overwriting an earlier, more precise explicit registration for the same opcode addresses) — opcodes `0xFC`/`0xFD` had been silently running the wrong, argument-dropping handler this whole time, not just in scan scripts. Built `src/import/NpcSpawnTable.lua` (same conventions as `EnemyStatTable`/`EnemySpeciesTable`, all 191 real external `NPC_*` names included) plus the matching `rom_profiles.lua` entry. Added a regression test that runs the real opcode bytes through the FULL `ScriptRuntime` registration path (the one thing every existing test happened to skip) and would have caught the bug. `luajit tests/run_tests.lua`: 588/588 (580 previous + 8 new). Still open: no live `Field.lua`/`EventSystem.lua` gameplay wiring yet (confirmed `ScriptRuntime.lua` isn't currently the live driver of any real in-game content at all), and which real script(s) reach `0xFC`/`0xFD` outside the known 1357-script corpus is still unidentified. See events.md's own 2026-08-20 "Lua reimplementation" entry for the full chain. **2026-08-20, same-day "ja mach das. geh es an" follow-up — live gameplay wiring done**: real, live-captured Goblin sprite data (same OAM-capture method as `willyScene`) added as `graphics.goblinTestScene`; wired into `Field.lua` as a genuinely second, additive, independently-alive `self.secondEnemy` (own contact-damage/attack-hit/death handling, NOT tied to the courtyard boss's own victory-sequence trigger, deliberately stationary/no death-explosion since no real movement/death data exists for it). Live-verified by booting the real LÖVE app against the real ROM (not just unit tests) — a real screenshot + `:debugState()` log confirm it renders at its correct position with independent HP tracking. A real, pre-existing, unrelated visual quirk (a red fallback rectangle during the player's own invincibility-flicker frames) was found and isolated via `git stash` during this verification — confirmed NOT caused by this work, left unfixed as out of scope. `luajit tests/run_tests.lua`: 592/592. Root blocker fully resolved: this project now has a second, real, fightable enemy in live gameplay, not just an emulator-level proof. See events.md's own 2026-08-20 "second, real, FIGHTABLE creature" entry. |
...
| P2 | **NEW — Level/XP system** | 🟡 **reward VALUES now real and mostly decoded (2026-08-18)**, WRAM accumulator + growth formula still open | No real WRAM address for a per-kill XP write is known yet, and the 2 (then 3) real search angles run 2026-08-16 came back negative — but only because the ONE currently-reachable fight (row 16 of `EnemyStatTable`) genuinely has `xp=0`, `gold=0`, now CONFIRMED by real ROM data, not just a live-tracing gap. 15 of the other 20 species/bosses have real, non-zero, cross-verified `xp` values (10-210) sitting ready. Same root blocker as Bestiary above — reach a second enemy, then re-run the already-proven live-diff method against THAT fight to find the real WRAM accumulator. **2026-08-20, direct follow-up (Level/XP chosen as next priority once Bestiary was solved)**: tried exactly that against the now-reachable live Goblin (2 real live tests, willyRoom and secondRoom). Located its real `$C200` entity-slot record (position fields match its own live-captured OAM position exactly); a real chase-and-attack script achieved genuine adjacency (distance 3) during an extended real combat attempt. Decisive negative: the record's only plausible numeric field pair (`PARAM6`/`PARAM7`) never changed once, and its `ALIVE` field never reached the real dead sentinel — cycling instead through values matching a facing/animation bitmask, not alive/dead. No candidate XP/gold write found in a broad WRAM diff either. Honest, undecided between two real explanations: this NPC-placement path may simply not support standard hostile combat at all (plausible given `NpcSpawnTable`'s own mixed monster/decoration naming), or this project's scripted attack input isn't reliably landing a registered hit on a moving target. Root blocker for Level/XP genuinely still open, now understood more precisely — see events.md's own 2026-08-20 "decisive negative" entry for the full trace and 2 concrete, not-yet-attempted next steps. |
