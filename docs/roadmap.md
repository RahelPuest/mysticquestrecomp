...
| P2 | **NEW — Magic/spell system** | 🟢 **which record is which spell + its real MP cost + the real ROM deduction code: CLOSED (2026-08-19)**; 🔴 in-combat spell EFFECTS and a real spell-learning trigger still unfound | The 8 MP-costed castable spells (Cure/Heal/Sleep/Mute/Fire/Ice/Lightning/Nuke) turned out to be `ItemTable` records 0-7 (previously wrongly assumed "found/thrown combat items, never sold") — found by live-disassembling the real ROM's own MP-deduction routine (bank 2 `$B18F`-`$B1AB`, resolves back to `categoryByte AND 0x1F` of the SAME already-decoded item table, byte 8), 8/8 external MP-cost matches. Website's item table (previously mislabeling this exactly backwards) corrected. Still open: what each spell actually DOES in combat, and how a fresh character ever learns one (live-verified via mGBA: the real menu's "Magie" option genuinely closes with zero effect for a character with no known spells — matches this project's own existing UI behavior). See events.md's own 2026-08-19 entry. |
