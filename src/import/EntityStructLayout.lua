...
EntityStructLayout.FIELD_ACCESSOR_ADDRESS = {
  ...
  -- $0CA6's setter is guarded: if the slot's old value was already the
  -- dead sentinel (0xFF), it writes the new value then immediately
  -- forces it back to 0xFF -- a "can't revive a dead slot through this
  -- specific setter" rule, distinct from the allocate routine (`$0A74`)
  -- which presumably bypasses it.
  [EntityStructLayout.FIELD.TYPE] = { get = 0x0C4F, set = 0x0C5D },
  -- Usage sample (bank 1, `$4B65`/`$4B78`/`$4FA9`): all 3 call sites
  -- found write TYPE for slot 4 (the player's slot) with small integers
  -- (`1`, `4`) alongside PARAM2 writes in the same functions.
  --
  -- RETRACTED (direct live-test follow-up): this doc comment previously
  -- read "reads like a dynamic per-frame state value... plausibly
  -- attack-related" -- a hypothesis built from the static shape alone
  -- (facing/direction-masked dispatch, a `$C4D2:=0x3C` timer write,
  -- `CALL $02AB` right after). Live-tested it directly: mGBA
  -- watchpoints on TYPE/PARAM2/PARAM3/PARAM6/PARAM7/UNKNOWN_10/
  -- UNKNOWN_11/`$C4D2` across an A-press attack
  -- (`courtyard_enemy_engaged` checkpoint) found zero value changes; a
  -- follow-up direct PC/execution check (600,000 single steps spanning
  -- the whole attack) found the handler blocks at all 3 TYPE-setter
  -- call sites (`$4B60`, `$4B72`, `$4F9D`) and their further dispatch
  -- targets (`$5010`, `$5084`) are never reached during a normal melee
  -- attack -- a clean, decisive, honest negative, not an inconclusive
  -- one. The "attack-related" hypothesis is wrong as stated. What
  -- remains true (byte-level, unaffected by this correction): the 3
  -- static call sites exist and do write small integers to slot 4's
  -- TYPE field. Whether this whole code region is genuinely dead code,
  -- or reached under some other condition this pass didn't test (a
  -- different room/game state, a move not available this early), stays
  -- open.
  [EntityStructLayout.FIELD.PARAM2] = { get = 0x0C6D, set = 0x0C86 },
  [EntityStructLayout.FIELD.POSITION_Y] = { get = 0x0C41, set = nil },
  -- Offsets 6/7 (`PARAM6`/`PARAM7`): no separate 1-byte accessors found
  -- in this block -- instead, `$0CBA` is a guarded 16-bit setter
  -- treating them as one paired field (`LD (HL),E / INC HL / LD (HL),D`
  -- at offset+6/+7 together, skipped entirely if the slot is dead).
  -- Evidence these two nominally-separate byte fields are, in practice,
  -- used as a single 16-bit value by at least this caller -- a
  -- refinement of the current FIELD table's split naming, not yet
  -- strong enough to rename them outright.
  PAIRED_6_7_SET = 0x0CBA,
  [EntityStructLayout.FIELD.UNKNOWN_10] = { get = 0x0CD3, set = 0x0CE4 },
  -- Cross-confirms the $4BE0/$278F "$C4E0 record's ID byte is used as
  -- a $C200 slot index" finding via a second, independent code path:
  -- `$404A` (a per-record tick handler, see ScriptOpcodeTable.lua)
  -- calls `$0CD3` with `C` = the record's ID byte, feeding the result
  -- into its own internal branch.
  [EntityStructLayout.FIELD.UNKNOWN_11] = { get = 0x0CF7, set = 0x0D08 },
}

-- CONFIRMED, no longer just a hypothesis: live-traced a "cut" room
-- transition (thirdRoom -> fourthRoom) end to end -- the landing-
-- position commit chain (`$4992: LD B,0x00 / LD C,0x04 / CALL $0611`,
-- see `src/import/TileLandingPosition.lua`'s doc comment for the full
-- trace) passes the literal value `4` as the entity slot index into
-- the same `$0611` position-commit routine this project already fully
-- disassembled for ordinary movement -- direct, live, decisive
-- confirmation the player really is slot 4, not just a structural
-- inference from address arithmetic.
EntityStructLayout.PLAYER_SLOT_INDEX_HYPOTHESIS = 4

-- CRACKED: the ROM leaf `$02AB` (shared dependency behind the
-- whole-corpus scan's known-hard `0x80`/`0xEC`/`0xED`/`0xEE`/`0x81`/
-- `0xA4` opcode family) is nothing more than `LD C,0x04 / CALL $0C99 /
-- RET`, where `$0C99` is a plain, general "read FIELD.ALIVE of slot C"
-- primitive (`HL = $C200+C*16 / A = *(HL)`). With `C` hardcoded to `4`
-- (the player's slot, per `PLAYER_SLOT_INDEX_HYPOTHESIS` above), `$02AB`
-- simply reads the player's `$C240` byte -- not an opaque, unmodelable
-- leaf.
--
-- Live-traced `$C240`'s value across idle/movement/attack (mGBA,
-- `courtyard_enemy_engaged()` checkpoint): the low nibble is a one-hot
-- facing-direction bitmask (confirmed decisively: the idle value
-- `0x04` exactly matches "facing up, bit `4`," matching this project's
-- independently-live-verified `Player.DEFAULT_FACING = "up"`; moving
-- right/left/up/down produced `0x11`/`0x12`/`0x14`/`0x18` -- i.e.
-- `1`/`2`/`4`/`8` in the low nibble). The upper nibble varies with
-- movement/attack sub-state (idle/walk-frame/attack-frame) but is
-- irrelevant to `0x80`'s formula (`AND 0x0F`), which masks it away
-- entirely -- `0x80`'s "dynamic group" is purely a function of the
-- player's current facing direction, independent of movement/attack
-- state. See events.md's "task 10" entry for the complete live-trace
-- data and the full disassembly.
...
-- CRACKED (opcode 0x81/$15B7): a general ROM helper (`$29E4`) takes a
-- `PLAYER_FACING_BIT`-shaped low nibble and returns the opposite
-- direction, via a bit trick: `AND 0x0F`, then XOR 0x03 on the low
-- pair (bits 0-1, right/left) whenever it's not already zero, XOR 0x0C
-- on the high pair (bits 2-3, up/down) whenever it's not already zero.
-- Worked out by truth table (only one-hot inputs matter, since
-- `$02AB`'s low nibble is always one-hot per the 0x80 investigation):
-- `0x01<->0x02` (right<->left), `0x04<->0x08` (up<->down), `0x00->0x00`.
-- `$29E4` is a general helper -- this table is this project's Lua-side
-- equivalent (a plain lookup is simpler and exactly as correct as
-- reproducing the bit trick, since the input is always one-hot in
-- every call site found so far).
...
--- General helper: the WRAM address of `field` within `slotIndex`.
-- Pure arithmetic, matches the ROM's `$C200 + slotIndex*16 + field`
-- computation exactly (both `$0AE3`'s 4x `ADD HL,HL` doubling and
-- `$0A74`'s equivalent `LD DE,0x0010` add reach the same address).
