-- Real port of ROM routine `$2B1E` (bank 0, always mapped) -- the PRNG
-- draw combat's damage/HP formulas use. Disassembled fresh (2026-08-10,
-- see docs/reverse-engineering/rom-map.md "$50AC, the real damage
-- formula") to get the EXACT algorithm, not a `math.random()` stand-in:
--
...
--
-- i.e. this is a real, COMBINED double-lookup PRNG (an advancing
-- position counter summed with a second, much-more-slowly-drifting
-- "cap" index that only decrements when the fast counter catches up to
-- it) -- not a plain single-counter table walk. Both internal state
-- bytes wrap mod 256, matching the real 8-bit registers exactly.
--
-- Real hardware's own starting `$C0B0`/`$C0B1` values (whatever WRAM
...
-- and doing so isn't the point: what matters is porting the real
-- ALGORITHM/shape, not one specific cartridge's boot-time seed. Both
-- counters start at 0 here, a clearly-documented choice, not a claimed
-- ROM fact.
--
...
--- One real PRNG draw (0-255), advancing internal state exactly like
...
function CombatNoise:draw()
  ...
  -- byte indices into the real 256-byte table, hence the +1.
