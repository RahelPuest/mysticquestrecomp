...
-- module existed. Extracted per direct user instruction to look for
-- general mechanisms rather than one hardcoded module per room.
--
...
-- every room this project has ground-truthed so far (the courtyard,
-- the post-victory room) turned out to load its own small, scattered
-- tile-graphics set into VRAM rather than referencing one shared
-- contiguous tileset by plain index -- assuming a flat stride here
-- already produced one confirmed-wrong render (`willyRoom`'s original
-- implementation, corrected same-session -- see rom-map.md's "Real
-- room-tile decompression pipeline" entry for the full story and the
-- general lesson: a plausible-looking render is not sufficient
-- confirmation of the tileset base).
--
-- The flat-stride alternative used to live in `RoomBackground.lua` (a
-- `MapTable`-based renderer for bank 5's own RLE map records) --
-- removed as dead code (never `require`d by anything, confirmed via a
-- full-repo sweep): the "how do multiple records compose into an
-- on-screen room" question it was built to answer stayed open at the
-- single-record level and was later actually resolved through a
-- different mechanism (Milestone 3's bank-5 room-table composition
-- breakthrough, see rom-map.md/progress.md), which this project's
-- actual room content (`rom_profiles.lua` + `Field.lua`) is built on
-- instead.
